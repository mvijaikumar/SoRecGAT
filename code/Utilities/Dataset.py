...
class Dataset(object):
    def __init__(self,args):
        ...
        if args.method.lower() in ['sorecgatitem','sorecgatuser']:
            self.item_embed_flag = True
        else:
            self.item_embed_flag = False
        ...
        if self.item_embed_flag == True: 
            ...
            if args.method.lower() in ['sorecgatitem','sorecgatuser']:
                #if args.cdcf == '1':
                #    item_embed_ext = ".cdcf_item_embed.final"
                #else:
                item_embed_ext = ".item_embed.final"
                self.item_embed_ext = item_embed_ext # for inheritance
                self.attr_dim = self.get_item_embed_dim(self.embed_path + item_embed_ext)
                self.attr_mat            = self.load_embed_as_mat(self.embed_path + item_embed_ext)
                self.item_embed_dict     = self.load_embed_file_as_dict(self.embed_path + item_embed_ext)
                if args.method.lower() in ['sorecgatitem','sorecgatuser']:# for user att_mat with the help of item attr_mat helper functions
                    #self.get_item_embed_dim(self.embed_path + item_embed_ext.replace('item','user')) #assumed user and item has same attr_dim
                    self.user_attr_mat       = self.load_embed_as_mat      (self.embed_path + item_embed_ext.replace('item','user'),flag='user')
                    self.user_embed_dict     = self.load_embed_file_as_dict(self.embed_path + item_embed_ext.replace('item','user'))
        ...
        # loading matrix of items interacted by users to apply attention on items hete embedding
        self.tar_dom         = self.get_target_domid(test_file=path_valid_test + ".test") ## only for analysing performance for target domain separated
    ...
    def load_embed_as_mat(self, filename,flag='item'):
        ...
        if flag == 'user':
            ...
            mat = np.zeros((self.num_item,self.attr_dim),dtype=np.float32)
        ind =0
        with open(filename, "r") as f:
            line = f.readline()
            while line != None and line != "":
