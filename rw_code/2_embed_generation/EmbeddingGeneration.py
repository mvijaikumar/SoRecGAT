...
def parse_args():
    ...
    parser.add_argument('--metapath_list',nargs='?', default='.UI', help='metapath lists with space dilimeter.') #default='.ID .IV .IB'
    parser.add_argument('--user_item', type=int, default=1,      help='1--> user-item 0--> for user-user or user-social .')
    
    #parser.add_argument('--output', nargs='?',default='sample2',help='Use <file> to save the resulting word vectors / word clusters.')
    #parser.add_argument('--item_embed_output',nargs='?',default='sample2', help='Use <file> to save the resulting only item vectors.')
    #parser.add_argument('--item_id', nargs='?',default='sample',help='Use <file> to save the resulting only item vectors.')    
    #===============
...
class EmbeddingGeneration:
    ...
    def store_embeddings(self, fin_name, fout_name):
        ...
        with open(fin_name) as fin, open(fout_name+'.user_embed.deepwalk','w') as fuser, open(fout_name+'.item_embed.deepwalk','w') as fitem:
            for line in fin:
                line = line.strip()
                wrds = line.split(' ')
                if 'I' == wrds[0][0]:
                    fitem.write(wrds[0].replace('I','')+'::'+' '.join(wrds[1:])+'\n')
                if 'U' == wrds[0][0]:
                    fuser.write(wrds[0].replace('U','')+'::'+' '.join(wrds[1:])+'\n')
...
if __name__ == '__main__':
    ...
    filenames = ['.embed.txt','.embed','.walks']
    for fil in filenames:
        subprocess.call(['rm',args.dir + args.dataset + fil])    
    if args.user_item == 1:
        subprocess.call(['mv',args.dir + args.dataset  + '.user_embed.deepwalk',args.dir + args.dataset.replace('.train','') + '.user_embed.final'])    
        subprocess.call(['mv',args.dir + args.dataset  + '.item_embed.deepwalk',args.dir + args.dataset.replace('.train','') + '.item_embed.final'])    
    print("Finished...")
