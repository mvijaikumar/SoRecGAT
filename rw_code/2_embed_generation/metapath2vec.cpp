...
//added 
std::vector<long long> I_table(0), U_table(0), V_table(0), D_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    if (vocab[target].word[0] == 'I') {
      I_table.push_back(target);
    } else if (vocab[target].word[0] == 'U') {
      U_table.push_back(target);
    } else if (vocab[target].word[0] == 'V') {
      V_table.push_back(target);
    } else if (vocab[target].word[0] == 'D') {
      D_table.push_back(target);
    }
  }
  ...
  std::cout << "D_table.size() " << D_table.size() << std::endl;
}
...
void *TrainModelThread(void *id) {
  ...
  //printf("iter inside : %lld ",local_iter);
    ...
    //train skip-gram
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          ...
          //printf("target: %s lastword: %s %s \n",vocab[last_word].word,vocab[target].word,vocab[word].word); //##I added
        }
...
int main(int argc, char **argv) {
  ...
  //print values
  printf("\n size: %lld", layer1_size);
  printf("\n train: %s",train_file);
  printf("\n save-vocab: %s",save_vocab_file);
  printf("\n read-vocab: %s",read_vocab_file);
  printf("\n debug: %d",debug_mode);
  printf("\n alpha: %f",alpha);
